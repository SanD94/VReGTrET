...
data_path = "D:\big-data\2025-westbrueck\preprocessing-pipeline\graphs";
...
% groups = ["Normal", "Central", "Peripheral"];
...
inside_node_number = zeros(Number, 1);
outside_node_number = zeros(Number, 1);
xor_node_number = zeros(Number, 1);
...
for ii = 1:Number
    ...
    inside_node_number(ii) = length(ic);
    outside_node_number(ii) = length(oc);
    xor_node_number(ii) = length(xc);
...
connectivity_table.inside_node_number = inside_node_number;
connectivity_table.outside_node_number = outside_node_number;
connectivity_table.xor_node_number = xor_node_number;
